        ...
        if ($msg->rowCount()) {
            ...
            foreach ($msg->fetchAll(PDO::FETCH_ASSOC) as $val) {
                ...
                if ($sender_color->rowCount() && $receiver_color->rowCount()) {
                    foreach ($sender_color->fetchAll(PDO::FETCH_OBJ) as $val)
                        $sender_color = $val->use_color;
                    foreach ($receiver_color->fetchAll(PDO::FETCH_OBJ) as $val)
                        $receiver_color = $val->use_color;
                } else {
